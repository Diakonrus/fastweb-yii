...
class NestedSetBehavior extends CActiveRecordBehavior
{
	 ...
	 * @param int $depth the depth.    - уровень вложености
	 * @return CActiveRecord the owner.
     * $status - статус узла
	 */
	public function descendants($depth=null, $status=null)
	{
		$owner=$this->getOwner();
        ...
        $status_where = "";
        if (!empty($status)){
            $status_where = 'status = '.(int)$status.' AND ';
        }

        //$criteria->select = 'id, name';

		$criteria->mergeWith(array(
			'condition'=>' '.$status_where.' '.$alias.'.'.$db->quoteColumnName($this->leftAttribute).'>'.$owner->{$this->leftAttribute}.
    ...
    public function rootsByLvl()
    {
        $owner=$this->getOwner();
        $db=$owner->getDbConnection();
        $owner->getDbCriteria()->addCondition($db->quoteColumnName($owner->getTableAlias()).'.'.$db->quoteColumnName($this->levelAttribute).'=1');
