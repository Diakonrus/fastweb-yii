...
class ModelCode extends CCodeModel
{
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
            ...
            // extra fields
            $params['fieldsInfo'] = $this->prepareFieldsInfo($params);
            $params['rules'] = $this->generateRules($table);
    ...
    /** extra fields */
    protected function prepareFieldsInfo(&$params) {
        $result = array(
            'extra'=>array(),
            'extraCommented'=>array(),
            'date'=>array(),
            'user'=>array(),
            'related_list'=>array(),
        );
        ...
        foreach($params['columns'] as $name=>$column)
        {
            if($name == 'user_id')
            {
                $result['user'][$name] = $column;
                $result['extraCommented'][$name.'s'] = $column;
                $params['labels'][$name.'s'] = 'Пользователи';
            }
            else if(substr($name, -3) == '_id')
            {
                $result['related_list'][$name] = $column;
            }
            else if(substr($name, -3) == '_at')
            {
                $result['date'][$name] = $column;
                $result['extra'][$name.'_start'] = $column;
                $result['extra'][$name.'_end'] = $column;
            }

            if(substr($name, 0, 3) == 'is_')
            {
                $params['columns'][$name]->type = 'boolean';
            }
            else if(stripos($column->dbType,'int')!==false)
            {
                $params['columns'][$name]->type = 'integer';
            }
        }
