...
<div class="control-group">
    <label class="control-label required" for="ArticleRubrics_name">
        Категория
        <span class="required">*</span>
    </label>
    <div class="controls">
        <select name="ArticleRubrics[parent_id]" id="ArticleRubrics_parent_id" class="span5" <?=!$model->isNewRecord?'disabled':'';?>>
...
<div class="control-group">
    <label class="control-label" for="ArticleElements_imagefile">Картинка раздела</label>
    <div class="controls">
        <?php if ($model->isNewRecord) { ?><img src="/images/nophoto_100_100.jpg"><?php } else {
            ...
            if (file_exists( YiiBase::getPathOfAlias('webroot').'/../uploads/filestorage/article/rubrics/admin-'.$model->id.'.'.$model->image )) {
                $url_img = '/../uploads/filestorage/article/rubrics/admin-'.$model->id.'.'.$model->image;
            }
            echo '<a href="/../uploads/filestorage/article/rubrics/'.$model->id.'.'.$model->image.'" target="_blank"><img src="'.$url_img.'"></a>';
        } ?>
        <br>
