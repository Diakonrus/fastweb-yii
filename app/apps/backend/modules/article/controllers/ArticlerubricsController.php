...
class ArticlerubricsController extends Controller
{
	...
	public function actionView($id)
	{
		...
		if(isset($_GET['ArticleRubrics'])){
			$model->attributes=$_GET['ArticleRubrics'];
        }
	...
	*/
	...
	public function actionCreate()
	{
		$this->breadcrumbs = array(
			'Статьи'=>array('/article/articlerubrics/index'),
			'Новая запись'
		);

		$model = new ArticleRubrics();
		$root = ArticleRubrics::getRoot($model);
		$descendants = $root->descendants()->findAll($root->id);

		if(isset($_POST['ArticleRubrics']))
		{
			$model = new ArticleRubrics;
			$parent_id = (int)$_POST['ArticleRubrics']['parent_id'];
			$root = ArticleRubrics::model()->findByPk($parent_id);
			$model->attributes = $_POST['ArticleRubrics'];
			...
			if (!$root){
				...
				$result = ArticleRubrics::getRoot(new ArticleRubrics);
			...
			if(!empty($model->id)){
				...
				if (isset($model->imagefile) && $modelSettings = SiteModuleSettings::model()->find('site_module_id = 6')){
					...
					$filepatch = '/../uploads/filestorage/article/rubrics/';
	...
	public function actionUpload(){
        ...
        $webFolder = '/uploads/articlerubrics/';
	...
	public function actionDelete($id = null)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			foreach($list as $id){
				ArticleElements::model()->deleteAll('parent_id = '.$id);
				//Удаляем категорию
	...
	{
		$dataProvider=new CActiveDataProvider('ArticleRubrics');
	...
	*/
	...
	public function actionIndex()
	{
		...
		$model = new ArticleRubrics;
		$category = ArticleRubrics::getRoot($model);
		$category = ArticleRubrics::model()->findByPk($category->id);
		...
		$provider=new CActiveDataProvider('ArticleRubrics', $data);
	...
	{
		$model=ArticleRubrics::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='article-rubrics-form')
		{
		...
		if (isset($_POST)) {
			switch ((int)$_POST['type']) {
				case 1:
					...
					foreach (ArticleElements::model()->findAll('parent_id = '.$model->id) as $data){
