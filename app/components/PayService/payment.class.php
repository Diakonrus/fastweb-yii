...
//Инициализируем класс	
	function  GetPayment()
	{
        ...
        if (!empty($MerchantId)) $this->MerchantId=$MerchantId;
        if (!empty($PrivateSecurityKey)) $this->PrivateSecurityKey=$PrivateSecurityKey;
        if (!empty($OrderId)) $this->OrderId=$OrderId;
        if (!empty($Amount)) $this->Amount=$Amount;
        if (!empty($Currency)) $this->Currency=$Currency;
        if (!empty($OrderDescription)) $this->OrderDescription=$OrderDescription;
        if (!empty($ValidUntil)) $this->ValidUntil=$ValidUntil;
        if (!empty($ReturnUrl)) $this->ReturnUrl=$ReturnUrl;
        if (!empty($FailUrl)) $this->FailUrl=$FailUrl;
        if (!empty($IData)) $this->IData=$IData;
        if (!empty($PNR)) $this->PNR=$PNR;
        if (!empty($Commission)) $this->Commission=$Commission;
        if (!empty($RebillAnchor)) $this->RebillAnchor=$RebillAnchor;
        if (!empty($TransactionId)) $this->TransactionId=$TransactionId;
        if (!empty($Ip)) $this->Ip=$Ip;
        if (!empty($Email)) $this->Email=$Email;
        if (!empty($CardHolderName)) $this->CardHolderName=$CardHolderName;
        if (!empty($CardNumber)) $this->CardNumber=$CardNumber;
        if (!empty($CardExpDate)) $this->CardExpDate=$CardExpDate;
        if (!empty($CardCvv)) $this->CardCvv=$CardCvv;
        if (!empty($Country)) $this->Country=$Country;
        if (!empty($City)) $this->City=$City;
        if (!empty($Address)) $this->Address=$Address;
        if (!empty($Zip)) $this->Zip=$Zip;
        if (!empty($State)) $this->State=$State;
        if (!empty($Phone)) $this->Phone=$Phone;
        if (!empty($Issuer)) $this->Issuer=$Issuer;
        if (!empty($ContentType)) $this->ContentType=$ContentType;
        if (!empty($pd)) $this->PD=$pd;
        if (!empty($pares)) $this->PARes=$pares;
        if (!empty($TermUrl)) $this->TermUrl=$TermUrl;
	}
