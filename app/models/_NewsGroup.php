...
class NewsGroup extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, name, description, url, brieftext, status, meta_title, meta_keywords, meta_description, created_at, param_design, created_at_start, created_at_end,
                   ', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function returnDesignGroup($model){
        ...
        $return = str_replace("%image%", ((!empty($model->image))?('<img src="/uploads/filestorage/news/rubrics/'.$model->id.'.'.$model->image.'">'):('')), $return);
        $return = str_replace("%brieftext%", $model->brieftext, $return);
        $return = str_replace("%description%", $model->description, $return);

        $news_element = '';
        $i = 0;
        foreach (News::model()->findAll('group_id = '.$model->id.' AND status!=0 ORDER BY id DESC') as $dataNews){
            ++$i;
            if ($i%2!=0){ $news_element.='<div class="press-line">'; }
            $news_element .= NewsGroup::model()->returnDesignElement($dataNews);
            if ($i%2==0){ $news_element.= '</div>'; }
        }
        $return = str_replace("%news_element%", $news_element, $return);
        return $return;
    }
