 ...
 *
 * The followings are the available columns in table '{{article_rubrics}}':
 * @property integer $id
 ...
 */
class ArticleRubrics extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, parent_id, left_key, right_key, level, url, image, name, title, description, description_short, status, meta_title, meta_keywords, meta_description, created_at, created_at_start, created_at_end,
                   ', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'articleElements' => array(self::HAS_MANY, 'ArticleElements', 'parent_id'),
		);
	}
	 ...
	 * @return ArticleRubrics the static model class
	 */
	...
	public static function getRoot(ArticleRubrics $model){
		$root = $model->roots()->find();
		if (! $root){
			$model->name = 'Каталог Article Дерево';
			$model->url = 'RootTreeArticle';
			$model->parent_id = 0;
			$model->level = 1;
			$model->status = 1;
			$model->saveNode();
	...
	public static function getCountTree($left_key, $right_key){
		$model = Yii::app()->db->createCommand()
			...
			->from(ArticleRubrics::model()->tableName())
	...
	public static function getCountElement($parent_id){
		return ArticleElements::model()->count("parent_id=:field", array("field" => $parent_id));
	}
}
