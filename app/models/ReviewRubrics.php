...
/**
 * This is the model class for table "{{review_rubrics}}".
 *
 * The followings are the available columns in table '{{review_rubrics}}':
 * @property integer $id
 ...
 * @property ReviewElements[] $reviewElements
 */
class ReviewRubrics extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'reviewElements' => array(self::HAS_MANY, 'ReviewElements', 'parent_id'),
		);
	}
	 ...
	 * @return ReviewRubrics the static model class
	 */
    ...
    public static function getRoot(ReviewRubrics $model){
        $root = $model->roots()->find();
        if (! $root){
            $model->name = 'Каталог Review Дерево';
            $model->url = 'RootTreeReview';
            $model->parent_id = 0;
            $model->level = 1;
            $model->status = 1;
            $model->saveNode();
    ...
    public static function getCountTree($left_key, $right_key){
        $model = Yii::app()->db->createCommand()
            ...
            ->from(ReviewRubrics::model()->tableName())
