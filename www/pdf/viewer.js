...
'use strict';
var myParam = location.search.split('urlPdf=')[1];

//Отправляем аяксом ID файла, получаем ответ
/*
$.ajax({
    type: 'POST',
    url: '/content/fayli/ajaxopen',
    dataType: "json",
    async: false,
    data: { id:myParam },
    success: function(data) {
        if (data['LoginForm_password'] != undefined || data == "access_err"){
            location.href = "/";
        }
        else if (data == "access_err"){
            location.href = "/content/fayli/noaccess";
        }
        else {
            myParam = data;
        }
